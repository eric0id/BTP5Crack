...
if [ -f "/usr/bin/apt-get" ];then
	...
	if [ "$isDebian" != "" ];then
		...
		wget --no-check-certificate -O install.sh https://raw.githubusercontent.com/liaochaopeng/bt-panel-5.9.1-beta/master/install-ubuntu.sh && sudo bash install.sh
		exit;
	fi
fi
...
btpanel_Url=https://raw.githubusercontent.com/liaochaopeng/bt-panel-5.9.1-beta/master/panel.zip
